...
import io.pit.common.model.Event;
import io.pit.gateway.kafka.AvroPublisher;
import io.pit.gateway.kafka.DlqPublisher;
import io.pit.gateway.config.PropsPolicy;
import io.pit.gateway.config.JsonSchemaValidator;
import io.pit.gateway.config.PolicyService;
import io.pit.gateway.config.PiiPolicy;
...
public class BatchController {
    ...
    public Mono<BatchResponse> batch(@RequestHeader(value = "content-encoding", required = false) String encoding,
        ...
        return bodyBytesMono.map(bytes -> {
            ...
            io.pit.gateway.config.PolicyService.Policy p = policyService.getPolicy(apiKey);
            io.pit.gateway.config.PiiPolicy.Overrides overrides = null;
            if (p != null && (p.piiEmail != null || p.piiPhone != null || p.piiIp != null || (p.denyKeys != null && !p.denyKeys.isEmpty()) || (p.maskKeys != null && !p.maskKeys.isEmpty()))) {
                overrides = new io.pit.gateway.config.PiiPolicy.Overrides();
    ...
    private io.pit.gateway.config.PiiPolicy.Mode parseMode(String s) {
        if (s == null) return null;
        return switch (s.toLowerCase()) {
            case "allow" -> io.pit.gateway.config.PiiPolicy.Mode.ALLOW;
            case "drop" -> io.pit.gateway.config.PiiPolicy.Mode.DROP;
            default -> io.pit.gateway.config.PiiPolicy.Mode.MASK;
        };
    }
    private io.pit.gateway.config.PiiPolicy.IpMode parseIpMode(String s) {
        if (s == null) return null;
        return switch (s.toLowerCase()) {
            case "allow" -> io.pit.gateway.config.PiiPolicy.IpMode.ALLOW;
            case "drop" -> io.pit.gateway.config.PiiPolicy.IpMode.DROP;
            default -> io.pit.gateway.config.PiiPolicy.IpMode.COARSE;
        };
    }
}
