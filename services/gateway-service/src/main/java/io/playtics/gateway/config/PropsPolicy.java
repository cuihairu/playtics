...
public class PropsPolicy {
    ...
    public PropsPolicy(Environment env, ObjectMapper om) {
        this.om = om;
        List<String> list = Binder.get(env).bind("playtics.props.allowlist", List.class).orElse(Collections.emptyList());
        this.allowlist = new HashSet<>();
        for (Object o : list) this.allowlist.add(String.valueOf(o));
        this.maxEventBytes = Binder.get(env).bind("playtics.event.maxBytes", Integer.class).orElse(65536);
        this.maxRequestBytes = Binder.get(env).bind("playtics.request.maxBytes", Integer.class).orElse(1048576);
    }
