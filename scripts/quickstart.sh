...
curl -sS -X POST 'http://localhost:8080/v1/batch' \
  -H 'x-api-key: pk_test_example' \
  -H 'content-type: application/x-ndjson' \
  --data-binary $'{"event_id":"01JDEMO001","event_name":"level_start","project_id":"p1","device_id":"d1","ts_client":'$(date +%s000)'}\n'{"event_id":"01JDEMO002","event_name":"level_complete","project_id":"p1","device_id":"d1","ts_client":'$(date +%s000)'}'

sleep 2

echo "[4/5] Build Flink jobs (enrich, sessions)"
./gradlew :jobs:flink:events-enrich-job:build :jobs:flink:sessions-job:build || true
