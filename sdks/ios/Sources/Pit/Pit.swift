...
public final class Pit {
    public static let shared = Pit()
    private init() {}

    private var opts: PitOptions? = nil
    ...
    public func initSDK(_ options: PitOptions) {
        ...
        self.ensureTimer()
        if options.debug { print("[Pit] init deviceId=\(deviceId) queued=\(queue.count)") }
    ...
    public func track(_ name: String, props: [String: Any]? = nil) -> String {
        ...
        if opts?.debug == true { print("[Pit] queued \(e.event_id) \(name)") }
    ...
    private func flushOnce() {
        ...
        URLSession.shared.dataTask(with: req) { _, resp, _ in
            if self.opts?.debug == true {
                ...
                print("[Pit] flush code=\(code) count=\(slice.count)")
            }
            sem.signal()
        }.resume()
